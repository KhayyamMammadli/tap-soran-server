...
function authRouter(prisma) {
    ...
    const registerSchema = zod_1.z
        ...
        phone: zod_1.z.string().trim().min(3).optional(),
        whatsapp: zod_1.z.string().trim().min(3).optional(),
    })
        .superRefine((val, ctx) => {
    ...
    `;
        const mail = await (0, mail_1.sendMail)({ to: emailNorm, subject, text, html });
        // Debug mode: return code for testing (disable in production!).
        const debug = process.env.OTP_DEBUG_RETURN_CODE === "1" || (!isSmtpConfigured() && process.env.NODE_ENV !== "production");
        if (!mail.sent) {
            if (debug) {
                return res.json({
                    ok: true,
    ...
    r.post("/register/verify-otp", async (req, res) => {
        const schema = zod_1.z.object({ email: zod_1.z.string().email(), code: zod_1.z.string().min(4).max(12) });
        const parsed = schema.safeParse(req.body);
        if (!parsed.success)
            return res.status(400).json({ error: parsed.error.flatten() });
        const emailNorm = parsed.data.email.trim().toLowerCase();
        ...
        const storedSchema = zod_1.z.object({
            role: zod_1.z.enum(["BUYER", "SELLER"]).transform((v) => v),
        ...
        const token = (0, jwt_1.signToken)(user.id, user.tokenVersion);
        res.json({ user, token });
    });
    ...
    r.post("/login", async (req, res) => {
        ...
        if (!ok)
            return res.status(401).json({ error: "Invalid credentials" });
        const token = (0, jwt_1.signToken)(user.id, user.tokenVersion || 0);
        res.json({
            user: {
                id: user.id,
                role: user.role,
