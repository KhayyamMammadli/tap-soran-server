...
function adminRouter(prisma) {
    ...
    const legalType = zod_1.z.enum(["PRIVACY", "TERMS"]);
    r.get("/legal/:type", async (req, res) => {
        const parsed = legalType.safeParse(String(req.params.type || "").toUpperCase());
        if (!parsed.success)
            return res.status(400).json({ error: "Invalid type" });
        const page = await prisma.legalPage.findUnique({ where: { type: parsed.data } });
        return res.json(page);
    });
    const legalUpdateSchema = zod_1.z.object({ title: zod_1.z.string().min(2).max(200), content: zod_1.z.string().min(10) });
    ...
    r.get("/reports", async (req, res) => {
        ...
        const normalized = rows.map((r) => ({ ...r, reportedUser: r.reportedUser ?? r.targetUser }));
    ...
    const complaintStatusSchema = zod_1.z.object({ status: zod_1.z.enum(["OPEN", "RESOLVED", "DISMISSED"]) });
    r.patch("/complaints/:id/status", async (req, res) => {
        const parsed = complaintStatusSchema.safeParse(req.body);
        if (!parsed.success)
            return res.status(400).json({ error: parsed.error.flatten() });
        try {
            const updated = await prisma.userComplaint.update({ where: { id: req.params.id }, data: { status: parsed.data.status } });
            return res.json(updated);
        }
        catch (e) {
    ...
    const reportStatusSchema = zod_1.z.object({ status: zod_1.z.enum(["OPEN", "RESOLVED", "DISMISSED"]) });
    r.patch("/reports/:id/status", async (req, res) => {
        const parsed = reportStatusSchema.safeParse(req.body);
        if (!parsed.success)
            return res.status(400).json({ error: parsed.error.flatten() });
        try {
            const updated = await prisma.messageReport.update({ where: { id: req.params.id }, data: { status: parsed.data.status } });
            return res.json(updated);
        }
        catch (e) {
            return res.status(400).json({ error: e?.message || "Update failed" });
    ...
    const freezeSchema = zod_1.z.object({ hours: zod_1.z.number().int().min(1).max(24 * 30), reason: zod_1.z.string().min(3).max(200) });
    ...
    const blockSchema = zod_1.z.object({ reason: zod_1.z.string().min(3) });
    const blockWithUntilSchema = zod_1.z.object({
        reason: zod_1.z.string().min(3).max(500),
        blockedUntil: zod_1.z.string().datetime().optional().or(zod_1.z.literal("")),
    });
    ...
    r.patch("/users/:id/block", async (req, res) => {
        ...
        try {
            ...
            if (u?.pushEnabled) {
                await (0, push_1.sendExpoPush)(u.expoPushToken, "Hesab bloklandı", `Səbəb: ${parsed.data.reason}`, {
                    type: "ACCOUNT_BLOCKED",
                    reason: parsed.data.reason,
                    blockedUntil: until ? until.toISOString() : null,
    ...
    r.post("/complaints/:id/block", async (req, res) => {
        ...
        const updated = await prisma.$transaction(async (tx) => {
            ...
            await tx.userComplaint.update({ where: { id: complaint.id }, data: { status: "RESOLVED" } });
    ...
    r.delete("/users/:id", async (req, res) => {
        ...
        if (target.role === "SUPER_ADMIN")
            return res.status(400).json({ error: "Can't delete SUPER_ADMIN" });
    ...
    const catSchema = zod_1.z.object({ name: zod_1.z.string().min(2) });
