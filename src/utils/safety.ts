...
export type SafetyCheckResult =
  | { ok: true }
  | { ok: false; code: SafetyReasonCode; evidence: string };

export function checkChatSafety(text: string): SafetyCheckResult {
  const t = norm(text);
  if (!t) return { ok: true };
...
export function humanizeSafetyReason(code: SafetyReasonCode) {
  switch (code) {
    case "DRUG_TRADE":
