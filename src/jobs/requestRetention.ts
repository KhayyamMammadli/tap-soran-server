...
export async function cleanupOldRequests(prisma: PrismaClient) {
  ...
  if (old.length === 0) return { deletedRequests: 0 };
...
export function startRequestRetentionJob(prisma: PrismaClient) {
  const run = async () => {
    try {
